...
public class UpdateAttendance
{
    ...
    public class Handler(IUserAccessor userAccessor, AppDbContext context) : IRequestHandler<Command, Result<Unit>>
    {
        ...
        {
            var activity = await context.Activities.Include(x => x.Attendees).ThenInclude(x => x.User).FirstOrDefaultAsync(x => x.Id == request.Id, cancellationToken);
            ...
            {
                activity.Attendees.Add(new ActivityAttendee { UserId = user.Id, ActivityId = activity.Id, IsHost = false });
            }
